...
# are generated by the background poller), so the model must never write a
# closing of its own; code owns the closing, at send.
_CLOSING_RULE_NO_CLOSING = """\
CLOSING:
...
- End your reply with its final substantive sentence. The sender's signature is \
appended automatically when the email is sent."""
...
class DraftGeneratorService:
    ...
    def generate(
        ...
        *,
        ...
        # closing_rule above — the sender's signature is appended at send.
        signoff_instruction = (
            "Do not add any closing, sign-off, name, or signature — end with your "
            "final substantive sentence. The sender's signature is appended "
