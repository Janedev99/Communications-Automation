...
- Per-user email signatures — whoever sends, their signature is appended (company signature as fallback and for auto-sent mail)
- An Analytics page: AI token usage vs. the daily budget, email volume, draft-workflow quality (edit rate), escalation trends
...
| Router | Highlights |
|--------|-----------|
| `auth` | Login/logout, current user, change password, personal signature (`PATCH /auth/me/signature`), user management (admin) |
...
| `system_settings` | Runtime toggles: `auto_send_enabled`, `company_signature` (admin) |
...
| `analytics` | Read-only aggregates for the Analytics page: token usage vs budget, email volume, draft workflow, escalations (`?days=7..365`) |
...
- **Signatures are a send-time concern**: drafts are generated by the background poller, where the eventual sender is unknown, so generation produces a signature-less body (the model is forbidden from writing its own closing). At send, the **sender's** personal signature (`users.signature`) is appended — falling back to the company signature (`system_settings.company_signature`), which is also what T1 auto-sent mail uses. See `backend/app/services/signatures.py`.
...
**13 tables** (Alembic migrations 001–018):

| Table | Purpose |
|-------|---------|
| `users` | Staff and admin accounts, incl. per-user email signature |
...
| `system_settings` | Runtime key/value toggles (`auto_send_enabled`, `company_signature`) |
...
See `docs/HANDOFF.md` for the migration & operations guide, and `docs/USER_GUIDE.md` for the end-user app guide + integrations setup.
